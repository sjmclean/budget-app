...
import { BackupIntegrityManager, BudgetPackageManager, SafeRestoreManager } from "../packages/budget-file/src/index.js";
import { derivePasswordSecret, verifyPasswordSecret } from "../packages/security/src/index.js";
...
const attachment = manager.attachments.addAttachment(packagePath, attachmentSource);
assert(existsSync(manager.attachments.getAttachmentPath(packagePath, attachment.storedFileName)), "Expected safe attachment path to resolve");
...
assert(integrity.verify(backup.path, manifest).ok, "Expected fresh backup manifest verification to pass");

rmSync(join(backup.path, attachment.relativePath));
const corrupt = integrity.verify(backup.path, manifest);
assert(!corrupt.ok && corrupt.missingFiles.includes(attachment.relativePath), "Expected missing attachment to fail backup verification");
...
assert(manager.validate(restoredPath).ok, "Expected safely restored package to validate");

let invalidTargetRejected = false;
try {
  restore.restoreBackup(secondBackup.path, join(root, "not-a-budget-folder"), secondManifest);
} catch {
  invalidTargetRejected = true;
}
assert(invalidTargetRejected, "Expected non-.budget restore target to be rejected");

const secret = derivePasswordSecret("correct horse battery staple");
assert(verifyPasswordSecret("correct horse battery staple", secret), "Expected scrypt password verification to pass");
assert(!verifyPasswordSecret("wrong password", secret), "Expected wrong password to fail verification");
