...
if (edited.memo !== "Edited" || edited.amount !== -1250)
  throw new Error("Expected transaction edit");
...
await service.edit({
  transactionId: transaction.id,
  memo: "Override ok",
  forceReconciledEdit: true,
});
const overridden = await transactionRepo.getById(transaction.id);
if (overridden?.memo !== "Override ok")
  throw new Error("Expected explicit reconciled edit override");
