...
if (
  !empty.summary.issues.some(
    (issue) => issue.code === "YNAB4_NO_INPUT" && issue.severity === "error",
  )
) {
...
if (
  !preview.summary.issues.some(
    (issue) => issue.code === "YNAB4_TRANSACTION_ZERO_AMOUNT",
  )
) {
...
console.log(
  "PASS: v1.2 YNAB4 preview reports empty input, missing dates and zero amount rows clearly",
);
