...
import {
  AccountType,
  BudgetParticipation,
  type ImportedBankTransaction,
} from "../packages/types/src/index.js";
import {
  SqliteBudgetRepository,
  SqliteAccountRepository,
  SqliteTransactionRepository,
  SqliteBankImportBatchRepository,
...
const db = createDatabase(
  join(mkdtempSync(join(tmpdir(), "v1214-bank-")), "bank.sqlite"),
);
...
if (result.createdTransactionIds.length !== 2)
  throw new Error("Expected two committed bank transactions");
if ((await txRepo.findByBudget(budget.id)).length !== 2)
  throw new Error("Expected committed transactions in repository");

const batch = await batchRepo.getBatch(result.batch.id);
if (!batch || batch.status !== "committed")
  throw new Error("Expected committed bank import batch");
if ((await batchRepo.findItems(result.batch.id)).length !== 2)
  throw new Error("Expected batch item audit records");

const undone = await service.undo(result.batch.id);
if (undone !== 2)
  throw new Error("Expected undo to affect two imported transactions");
const afterUndo = await txRepo.findByBudget(budget.id);
if (!afterUndo.every((tx) => tx.isDeleted))
  throw new Error("Expected undo to soft-delete imported transactions");
