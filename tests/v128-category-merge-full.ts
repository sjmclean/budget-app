...
const budget = createBudget("v1.2.8 Category Merge Full", "AUD");
await budgetRepo.create(budget);
const month = createBudgetMonth(budget.id, "2026-06");
await budgetMonthRepo.create(month);
const group = createCategoryGroup(budget.id, "Living", 1);
...
const sourceMonth = createCategoryMonth(month.id, source.id);
await categoryMonthRepo.create({
  ...sourceMonth,
  assigned: 5000,
  activity: -1200,
  available: 3800,
});
const targetMonth = createCategoryMonth(month.id, target.id);
await categoryMonthRepo.create({
  ...targetMonth,
  assigned: 1000,
  activity: -200,
...
if (result.movedCategoryMonths !== 1 || result.movedGoals !== 1)
  throw new Error("Expected category month values and goals to move");
const mergedMonth = await categoryMonthRepo.getByBudgetMonthAndCategory(
  month.id,
  target.id,
);
if (
  !mergedMonth ||
  mergedMonth.assigned !== 6000 ||
  mergedMonth.activity !== -1400 ||
  mergedMonth.available !== 4600
)
  throw new Error("Expected merged category-month totals");
const goals = await goalRepo.findByCategory(target.id);
if (goals.length !== 1)
  throw new Error("Expected goal to move to target category");
