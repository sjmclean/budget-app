...
await txRepo.create(t1);
await txRepo.create(t2);
const result = await bulk.bulkUpdate({
  transactionIds: [t1.id, t2.id],
  clearedStatus: ClearedStatus.Cleared,
});
if (result.updated.length !== 2 || result.skipped.length !== 0)
  throw new Error("Expected both transactions to bulk update");
const updated = await txRepo.getById(t1.id);
if (updated?.clearedStatus !== ClearedStatus.Cleared)
  throw new Error("Expected transaction to be cleared");
const del = await bulk.bulkDelete([t1.id, "missing"]);
if (del.updated.length !== 1 || del.skipped.length !== 1)
  throw new Error("Expected partial bulk delete result");
