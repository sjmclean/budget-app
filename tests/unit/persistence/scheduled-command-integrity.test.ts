...
test("create, update, delete and advance commit exactly one scheduled mutation", async () => {
  const h = harness(); await h.commands.createScheduledTransaction(budgetId, input());
  ...
  assert.equal(h.committed.length, 4); assert.deepEqual(h.changes, Array(4).fill(["scheduled-transactions"]));
});
...
test("failed scheduled write records no committed scope or worker mutation", async () => {
  ...
  assert.equal(h.allocated.length, 1); assert.equal(h.committed.length, 0); assert.equal(h.changes.length, 0);
});
...
test("history and Enter share one exact operation group and preserve generated attachment", async () => {
  ...
  assert.deepEqual(h.allocated.map(({ mutationId }) => mutationId), h.committed.map(({ mutationId }) => mutationId));
  assert.equal(result.transaction?.transactions[0]?.id, "transaction-id");
  const attachment = result.transaction?.attachments[0]; assert.equal(attachment?.id, "transaction-id:attachment:template");
