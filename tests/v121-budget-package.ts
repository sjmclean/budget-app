...
assert(
  existsSync(join(packagePath, "budget.db")),
  "Expected budget.db to exist",
);
assert(
  existsSync(join(packagePath, "budget.json")),
  "Expected budget.json to exist",
);
assert(
  existsSync(join(packagePath, "Attachments")),
  "Expected Attachments folder to exist",
);
assert(
  existsSync(join(packagePath, "Backups")),
  "Expected Backups folder to exist",
);
...
assert(
  validation.ok,
  `Expected package validation to pass: ${validation.issues.join(", ")}`,
);

const opened = manager.open(packagePath);
assert(
  opened.metadata.name === "Household Budget",
  "Expected package opener to read metadata",
);
assert(
  opened.databasePath.endsWith("budget.db"),
  "Expected opener to return database path",
);

const metadata = JSON.parse(
  readFileSync(join(packagePath, "budget.json"), "utf8"),
);
