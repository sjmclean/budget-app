...
const budget = createBudget("v1.2.6 Registry", "AUD"); await budgetRepo.create(budget);
...
await registry.registerBudget("user-1", "/Budgets/Household.budget", "Household");
...
if (budgets.length !== 2) throw new Error("Expected multiple budgets in registry");
const tx = createTransaction({ budgetId: budget.id, accountId: account.id, date: "2026-06-17", amount: -1000, clearedStatus: ClearedStatus.Cleared });
await txRepo.create(tx);
const filtered = await search.filterTransactions({ budgetId: budget.id, clearedStatus: ClearedStatus.Cleared, amountMax: -500 });
if (filtered.length !== 1) throw new Error("Expected transaction search filters");
