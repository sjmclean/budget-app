...
const backup = manager.backups.createBackup(
  packagePath,
  "manual",
...
assert(
  manager.restore.verifyBackup(backup.path),
  "Expected backup verification to pass",
);
assert(
  existsSync(join(backup.path, attachment.relativePath)),
  "Expected backup to include attachments",
);

manager.restore.restoreBackup(backup.path, restoredPath);
assert(
  manager.validate(restoredPath).ok,
  "Expected restored package to validate",
);
assert(
  existsSync(join(restoredPath, attachment.relativePath)),
  "Expected restored package to include attachment",
);

for (let i = 0; i < 12; i += 1) {
  manager.backups.createBackup(
    packagePath,
    "auto",
    new Date(Date.UTC(2026, 5, 17, 11, i, 0)),
  );
}
const deleted = manager.backups.pruneBackups(packagePath, 10);
assert(deleted >= 3, "Expected backup pruning to delete old backups");
assert(
  manager.backups.listBackups(packagePath).length === 10,
  "Expected latest 10 backups to remain",
);
