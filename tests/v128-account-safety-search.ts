...
if (report.canClose)
  throw new Error("Expected account with transactions to be unsafe to delete");
const found = await search.searchTransactions({
  budgetId: budget.id,
  accountId: account.id,
  clearedStatus: ClearedStatus.Cleared,
  dateFrom: "2026-06-01",
...
if (found.length !== 1 || found[0].id !== tx.id)
  throw new Error("Expected indexed DB-backed transaction search result");
