...
let budgetMonth = addIncomeToBudgetMonth(createBudgetMonth("budget", "2026-06"), 400000); let groceries = createCategoryMonth(budgetMonth.id, "groceries"); const assignment = assignToCategoryMonth(budgetMonth, groceries, 50000); groceries = applyActivityToCategoryMonth(assignment.categoryMonth, -15000); console.log({ readyToBudget: assignment.budgetMonth.readyToBudget, groceries });
