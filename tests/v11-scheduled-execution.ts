...
async function main() {
  ...
  const service = new ScheduledTransactionExecutionService(scheduledRepo, txRepo);
  ...
  await scheduledRepo.create(createScheduledTransaction({
    budgetId: budget.id,
  ...
  if (executed.length !== 1) throw new Error(`Expected 1 due scheduled transaction, got ${executed.length}`);

  const transactions = await txRepo.findByBudget(budget.id);
  if (transactions.length !== 1) throw new Error(`Expected 1 materialised transaction, got ${transactions.length}`);
  if (transactions[0].amount !== -5000) throw new Error(`Expected amount -5000, got ${transactions[0].amount}`);

  const updatedSchedules = await scheduledRepo.findActiveByBudget(budget.id);
  if (updatedSchedules.length !== 1) throw new Error("Expected schedule to remain active after monthly advancement");
  if (updatedSchedules[0].nextDueDate !== "2026-07-01") {
    throw new Error(`Expected next due date 2026-07-01, got ${updatedSchedules[0].nextDueDate}`);
  }

  const secondRun = await service.executeDue(budget.id, "2026-06-17");
  if (secondRun.length !== 0) throw new Error("Expected no duplicate execution after due date advancement");
