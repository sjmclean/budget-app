...
await importMapRepo.create(
  createImportMap({
    importRunId: run.id,
    sourceEntityType: "Account",
    sourceEntityId: "ynab-account-1",
...
await audit.record({
  ...
  entityId: tx.id,
  payload: { source: "YNAB4" },
});

const result = await rollback.undoImportRun(run.id);
if (result.status !== "rolled_back")
  throw new Error("Expected import run rollback status");
if ((await txRepo.getById(tx.id)) !== null)
  throw new Error("Expected imported transaction to be removed");
if ((await accountRepo.getById(account.id)) !== null)
  throw new Error("Expected imported account to be removed");
