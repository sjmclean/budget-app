  ...
  await importer.import(
    {
      ...
      budgetCsv:
        "Month,Category,Budgeted,Outflows,Balance\n2026-06,Food,100.00,10.00,90.00",
    },
...
if (!failed)
  throw new Error(
    "Import should have failed because the trigger aborts transaction inserts",
  );

const importedAccounts = await db
  .select()
  .from(accounts)
...
const importedTransactions = await db
  .select()
  .from(transactions)
  .where(eq(transactions.budgetId, budget.id));
if (importedAccounts.length !== 0 || importedTransactions.length !== 0) {
  throw new Error(
    "Failed import should roll back accounts and transactions created earlier in the import",
  );
}
