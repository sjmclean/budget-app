...
const db = createDatabase(
  join(mkdtempSync(join(tmpdir(), "v1214-rules-")), "rules.sqlite"),
);
const rules = new PersistentPayeeRuleApplicationService(
  new SqlitePayeeRuleRepository(db),
);
...
await rules.create({
  ...baseRule,
  id: "rule-woolies-duplicate",
  name: "Woolworths duplicate",
});

const loaded = await rules.list("budget-1");
if (loaded.length !== 2)
  throw new Error("Expected persisted payee rules to round-trip");
if (loaded[0].priority !== 100)
  throw new Error("Expected rules to be priority ordered");

const conflicts = await rules.detectConflicts("budget-1");
if (conflicts.length !== 1)
  throw new Error("Expected same-pattern same-priority rule conflict");

await rules.update({ ...baseRule, pattern: "WOOLIES", priority: 50 });
const enabled = await rules.listEnabled("budget-1");
if (!enabled.some((rule) => rule.pattern === "WOOLIES"))
  throw new Error("Expected updated rule to persist");

await rules.delete("rule-woolies-duplicate");
if ((await rules.list("budget-1")).length !== 1)
  throw new Error("Expected deleted rule to be removed");
