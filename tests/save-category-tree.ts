...
async function main() { resetDatabase(); const db = createDatabase("Test.budget"); const budgetRepo = new SqliteBudgetRepository(db); const groupRepo = new SqliteCategoryGroupRepository(db); const categoryRepo = new SqliteCategoryRepository(db); const budget = createBudget("Household Budget"); await budgetRepo.create(budget); const housing = createCategoryGroup(budget.id, "Housing"); await groupRepo.create(housing); await categoryRepo.create(createCategory(housing.id, "Rent")); console.log(await groupRepo.findByBudget(budget.id)); console.log(await categoryRepo.findByGroup(housing.id)); }
main();
