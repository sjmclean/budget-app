...
if (plan.length < 8) throw new Error("Expected a meaningful foreign-key migration plan");
if (!plan.some((step) => step.table === "transactions" && step.references.includes("accounts"))) {
  throw new Error("Expected transactions FK plan to include account references");
}
