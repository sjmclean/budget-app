...
async function main() {
  ...
  if (firstRun.length !== 1 || firstRun[0] !== 11) {
    throw new Error(
      `Expected migration 11 to run once, got ${JSON.stringify(firstRun)}`,
    );
  }

  const records = await repo.findByVersion("11");
  if (records.length !== 1)
    throw new Error(`Expected 1 migration record, got ${records.length}`);
  ...
  if (secondRun.length !== 0) {
    throw new Error(
      `Expected already-applied migration to be skipped, got ${JSON.stringify(secondRun)}`,
    );
  }
