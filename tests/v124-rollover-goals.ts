...
const rolledGroceries = rollover.categoryMonths.find(
...
if (!rolledGroceries || rolledGroceries.previousAvailable !== 0) {
  throw new Error(
    "Expected overspent category not to carry negative available forward",
  );
}

if (rollover.budgetMonth.readyToBudget !== -5000) {
  throw new Error(
    "Expected cash overspending to reduce next month Ready To Assign",
  );
}
...
if (
  progress.remainingAmount !== 90000 ||
  progress.suggestedMonthlyContribution !== 15000
) {
  throw new Error(
    "Expected target-date goal to calculate remaining amount over remaining months",
  );
}
