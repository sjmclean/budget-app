...
if (preview.summary.accounts !== 2) throw new Error(`Expected 2 accounts, got ${preview.summary.accounts}`);
if (preview.summary.transactions !== 4) throw new Error(`Expected 4 transactions, got ${preview.summary.transactions}`);
if (preview.summary.transfers !== 1) throw new Error(`Expected 1 transfer, got ${preview.summary.transfers}`);
if (preview.summary.splitTransactions !== 1) throw new Error(`Expected 1 split transaction, got ${preview.summary.splitTransactions}`);
if (preview.summary.payees !== 3) throw new Error(`Expected 3 non-transfer payees, got ${preview.summary.payees}`);
if (!preview.categories.some((category) => category.groupName === "Everyday" && category.name === "Groceries")) {
...
console.log("PASS: v1.2 YNAB4 preview parses accounts, transactions, payees, categories, transfers and splits");
