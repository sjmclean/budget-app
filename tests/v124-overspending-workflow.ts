...
const groceries = createCategoryMonth(
  currentMonth.id,
  "groceries",
  0,
  20000,
...
const coveredGroceries = coverResult.categoryMonths.find(
  (cm) => cm.categoryId === "groceries",
);
const coveredDining = coverResult.categoryMonths.find(
  (cm) => cm.categoryId === "dining",
);

if (!coveredGroceries || coveredGroceries.available !== 0) {
  throw new Error(
    "Expected cover decision to bring overspent category to zero available",
  );
}

if (!coveredDining || coveredDining.available !== 5000) {
  throw new Error(
    "Expected cover decision to reduce covering category available",
  );
}
...
if (
  leaveResult.nextBudgetMonth.income !== 45000 ||
  leaveResult.nextBudgetMonth.readyToBudget !== 45000
) {
  throw new Error(
    "Expected leave-overspent decision to reduce next month Ready To Assign",
  );
}
