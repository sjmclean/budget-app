...
const sqlite = new Database(join(mkdtempSync(join(tmpdir(), "v1212-search-")), "search.sqlite"));
initDatabase(sqlite);

sqlite.prepare("INSERT INTO budgets (id, name, currency, created_at) VALUES (?, ?, ?, ?)").run("b1", "Budget", "AUD", Date.now());
sqlite.prepare("INSERT INTO accounts (id, budget_id, name, type, participation, opening_balance, current_balance) VALUES (?, ?, ?, ?, ?, ?, ?)").run("a1", "b1", "Everyday", "checking", "on-budget", 0, 0);
sqlite.prepare("INSERT INTO payees (id, budget_id, name, normalized_name, is_archived, is_transfer, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").run("p1", "b1", "Woolworths", "woolworths", 0, 0, Date.now(), Date.now());
sqlite.prepare("INSERT INTO transaction_tags (id, budget_id, name, created_at) VALUES (?, ?, ?, ?)").run("tag1", "b1", "Groceries", Date.now());

const insert = sqlite.prepare(`INSERT INTO transactions (id, budget_id, account_id, payee_id, category_id, type, date, memo, amount, cleared_status, is_deleted, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
for (let i = 1; i <= 5; i += 1) {
