...
const edited = { ...transaction, memo: "Edited memo", amount: -3000, updatedAt: new Date() };
...
if (!afterUndo || afterUndo.memo !== "Original memo" || afterUndo.amount !== -2500) {
...
if (!afterRedo || afterRedo.memo !== "Edited memo" || afterRedo.amount !== -3000) {
...
await undoRedo.recordCommand({
  ...
  redoPayload: { action: "softDeleteTransaction", transactionId: transaction.id }
});
