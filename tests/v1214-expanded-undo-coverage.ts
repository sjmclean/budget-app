...
const db = createDatabase(
  join(mkdtempSync(join(tmpdir(), "v1214-undo-")), "undo.sqlite"),
);
...
if (!undone || undone.memo !== "before" || undone.amount !== -500)
  throw new Error("Expected executable undo to restore transaction state");

await undoRedo.redoLast(budget.id);
const redone = await txRepo.getById(tx.id);
if (!redone || redone.memo !== "after" || redone.amount !== -750)
  throw new Error(
    "Expected executable redo to restore edited transaction state",
  );
