...
await db.insert(budgets).values({
  id: budgetId,
  name: "Import Report Budget",
  currency: "AUD",
  createdAt: new Date(),
});

const service = new Ynab4DatabaseImportService(db);
const dryRun = await service.import(
  {
    registerCsv: `Account,Date,Payee,Category,Memo,Outflow,Inflow,Cleared
Checking,,Missing Date,Everyday:Groceries,,1.00,,`,
  },
  { budgetId, dryRun: true },
);
if (dryRun.status !== "dry-run") throw new Error("Expected dry-run status");
if (
  !dryRun.issues.some(
    (issue) => issue.code === "YNAB4_TRANSACTION_MISSING_DATE",
  )
)
  throw new Error("Expected missing date issue in dry-run");
...
if (runs[0].status !== "completed")
  throw new Error(`Unexpected import status ${runs[0].status}`);
const summary = JSON.parse(runs[0].summaryJson);
if (summary.created.transactions !== 1)
  throw new Error("Import summary did not record created transaction count");

const maps = await db
  .select()
  .from(importMaps)
  .where(eq(importMaps.importRunId, result.importRunId));
if (!maps.find((map) => map.targetEntityType === "transaction"))
  throw new Error("Transaction import map missing");
const transactionRows = await db
  .select()
  .from(transactions)
  .where(eq(transactions.budgetId, budgetId));
if (transactionRows.length !== 1)
  throw new Error("Expected exactly one imported transaction");

console.log(
  "PASS: v1.2.3 records import runs, import maps, dry-run validation, and completion reports",
);
