...
if (!clean.foreignKeysEnabled)
  throw new Error("Expected SQLite foreign key enforcement to be enabled");
if (clean.quickCheck !== "ok")
  throw new Error(`Expected quick_check ok, got ${clean.quickCheck}`);
if (clean.missingIndexes.length > 0)
  throw new Error(`Missing v1.2.9 indexes: ${clean.missingIndexes.join(", ")}`);
if (clean.orphanIssues.length > 0)
  throw new Error(`Expected clean database to have no orphan issues`);
...
if (damaged.orphanIssues.length === 0)
  throw new Error("Expected orphan issues to be detected");
if (
  !damaged.orphanIssues.some(
    (issue) => issue.code === "orphan_transaction_budget",
  )
) {
...
if (
  !damaged.orphanIssues.some(
    (issue) => issue.code === "orphan_transaction_account",
  )
) {
