...
function issue(
  severity: Ynab4ImportIssue["severity"],
  code: string,
  message: string,
  source?: string,
  rowNumber?: number,
...
export function previewYnab4Import(
  input: Ynab4ImportInput,
): Ynab4ImportPreview {
  const issues: Ynab4ImportIssue[] = [];
  const accountCsv = input.accountsCsv
    ? parseCsvWithHeaders(input.accountsCsv)
    : { headers: [], rows: [] };
  const registerCsv = input.registerCsv
    ? parseCsvWithHeaders(input.registerCsv)
    : { headers: [], rows: [] };
  const budgetCsv = input.budgetCsv
    ? parseCsvWithHeaders(input.budgetCsv)
    : { headers: [], rows: [] };

  if (!input.accountsCsv && !input.registerCsv && !input.budgetCsv) {
    issues.push(
      issue("error", "YNAB4_NO_INPUT", "No YNAB4 CSV input was supplied."),
    );
  }

  if (input.accountsCsv)
    issues.push(
      ...detectYnab4Columns(accountCsv.headers, "accountsCsv").issues,
    );
  if (input.registerCsv)
    issues.push(
      ...detectYnab4Columns(registerCsv.headers, "registerCsv").issues,
    );
  if (input.budgetCsv)
    issues.push(...detectYnab4Columns(budgetCsv.headers, "budgetCsv").issues);
  ...
  const transactions = registerCsv.rows.map((row, index) =>
    mapYnab4RegisterRow(row, index + 2),
  );
  ...
  for (const transaction of transactions) {
    if (!transaction.date)
      issues.push(
        issue(
          "error",
          "YNAB4_TRANSACTION_MISSING_DATE",
    ...
    if (!transaction.accountName)
      issues.push(
        issue(
          "warning",
          "YNAB4_TRANSACTION_MISSING_ACCOUNT",
          "Transaction is missing an account name.",
          "registerCsv",
          transaction.rowNumber,
        ),
      );
  ...
  const allAccounts = uniqueBy([...accounts, ...inferredAccounts], (account) =>
    account.name.toLowerCase(),
  );

  const budgetMonths = budgetCsv.rows
    .map(mapYnab4BudgetRow)
    .filter((row) => row.category || row.month !== "unknown");
  ...
  const summary: Ynab4ImportSummary = {
    ...
    splitTransactions: transactions.filter((transaction) => transaction.isSplit)
      .length,
    transfers: transactions.filter((transaction) => transaction.isTransfer)
      .length,
    scheduledTransactions: 0,
    budgetMonths: uniqueBy(
      budgetMonths,
