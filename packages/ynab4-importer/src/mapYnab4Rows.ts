import { Ynab4TransactionPreview, Ynab4AccountPreview, Ynab4BudgetMonthPreview } from "../../types/src/Ynab4Import.js";
...
function value(row: Record<string, string>, ...keys: string[]): string {
  for (const key of keys) {
    ...
    const found = Object.entries(row).find(([candidate]) => candidate.trim().toLowerCase() === key.trim().toLowerCase());
...
export function mapYnab4RegisterRow(row: Record<string, string>, rowNumber = 0): Ynab4TransactionPreview {
  ...
  return {
    ...
    amount,
    cleared: clearedRaw === "R" ? "reconciled" : clearedRaw === "C" ? "cleared" : "uncleared",
    flag: value(row, "Flag") || null,
    isTransfer: Boolean(transferMatch),
    transferAccountName: transferMatch?.[1]?.trim() ?? null,
    isSplit: (category ?? "").toLowerCase().includes("split") || payee.toLowerCase().includes("split")
  };
}

export function mapYnab4AccountRow(row: Record<string, string>): Ynab4AccountPreview {
  ...
  return {
    name,
    type,
    onBudget: budgeted ? ["true", "yes", "on budget", "budget"].includes(budgeted) : null,
    balance: value(row, "Balance", "Current Balance") ? parseYnabAmount(value(row, "Balance", "Current Balance")) : null,
    closed: ["true", "yes", "closed"].includes(closed)
  };
}

export function splitCategoryName(category: string): { groupName: string | null; name: string; fullName: string } {
  ...
  return { groupName: groupName.trim() || null, name: name || trimmed, fullName: trimmed };
}

export function mapYnab4BudgetRow(row: Record<string, string>): Ynab4BudgetMonthPreview {
