...
function mapAccountType(
  ...
  if (["checking", "chequing", "current"].includes(normalized))
  ...
  if (["creditcard", "credit", "card"].includes(normalized))
    return AccountType.CreditCard;
  if (["investment", "brokerage"].includes(normalized))
    return AccountType.Investment;
  if (["asset"].includes(normalized)) return AccountType.Asset;
  if (["liability", "loan", "mortgage"].includes(normalized))
...
export class Ynab4DatabaseImportService {
  ...
  async import(
    input: Ynab4ImportInput,
    options: Ynab4DatabaseImportOptions,
  ): Promise<Ynab4DatabaseImportResult> {
    if (options.dryRun) {
    ...
    return this.db.transaction((tx: any) => {
      return new Ynab4DatabaseImportService(tx).importWithoutTransaction(
        input,
        options,
      );
    });
  ...
  private importWithoutTransaction(
    input: Ynab4ImportInput,
    options: Ynab4DatabaseImportOptions,
  ): Ynab4DatabaseImportResult {
    ...
    const result: Ynab4DatabaseImportResult = {
      ...
      status: options.dryRun
        ? "dry-run"
        : preview.summary.issues.some((issue) => issue.severity === "error")
    ...
    const existingAccounts = this.db
      .select()
      .from(accounts)
      .where(eq(accounts.budgetId, options.budgetId))
      .all();
    ...
    for (const account of preview.accounts) {
      ...
      this.db
        ...
        .values({
          ...
          type: mapAccountType(
            account.type,
            options.defaultAccountType ?? AccountType.Checking,
          ),
          participation:
            account.onBudget === false
              ? BudgetParticipation.OffBudget
              : (options.defaultParticipation ?? BudgetParticipation.OnBudget),
    ...
    const existingGroups = this.db
      .select()
      .from(categoryGroups)
      .where(eq(categoryGroups.budgetId, options.budgetId))
      .all();
    for (const group of existingGroups)
      groupMap.set(normalizeKey(group.name), group.id);
    ...
    for (const groupId of groupMap.values()) {
      ...
      for (const category of rows)
        categoryMap.set(
          `${groupId}:${normalizeKey(category.name)}`,
          category.id,
        );
    }
    ...
    for (const category of preview.categories) {
      const groupKey = category.groupName
        ? normalizeKey(category.groupName)
        : fallbackGroupKey;
    ...
    const existingPayees = this.db
      .select()
      .from(payees)
      .where(eq(payees.budgetId, options.budgetId))
      .all();
    const payeeMap = new Map<string, string>(
      existingPayees.map((payee: any) => [
        payee.normalizedName || normalizePayeeName(payee.name),
        payee.id,
      ]),
    );
    ...
    for (const row of preview.transactions) {
      const accountId = row.accountName
        ? accountMap.get(normalizeKey(row.accountName))
        : undefined;
      ...
      const categoryId = row.category
        ? (categoryFullNameMap.get(normalizeKey(row.category)) ?? null)
        : null;
      const transferAccountId = row.transferAccountName
        ? (accountMap.get(normalizeKey(row.transferAccountName)) ?? null)
        : null;
      const payeeId =
        row.isTransfer && row.transferAccountName
          ? (transferPayeeMap.get(normalizeKey(row.transferAccountName)) ??
            null)
          : row.payee
            ? (payeeMap.get(normalizePayeeName(row.payee)) ?? null)
            : null;
      const type = row.isSplit
    ...
    for (const row of preview.budgetMonths) {
      ...
      const categoryId = row.category
        ? categoryFullNameMap.get(normalizeKey(row.category))
        : undefined;
