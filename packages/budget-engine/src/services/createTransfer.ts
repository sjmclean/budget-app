...
export function createTransfer(input: CreateTransferInput): TransferPair {
  ...
  return {
    outflow: { id: `${transferId}:out`, budgetId: input.budgetId, accountId: input.fromAccountId, payeeId: null, categoryId: null, transferAccountId: input.toAccountId, type: TransactionType.Transfer, date: input.date, memo: input.memo ?? null, checkNumber: normaliseCheckNumber(input.checkNumber), amount: -input.amount, clearedStatus: input.clearedStatus ?? ClearedStatus.Uncleared, isDeleted: false, createdAt: now, updatedAt: now },
    inflow: { id: `${transferId}:in`, budgetId: input.budgetId, accountId: input.toAccountId, payeeId: null, categoryId: null, transferAccountId: input.fromAccountId, type: TransactionType.Transfer, date: input.date, memo: input.memo ?? null, checkNumber: normaliseCheckNumber(input.checkNumber), amount: input.amount, clearedStatus: input.clearedStatus ?? ClearedStatus.Uncleared, isDeleted: false, createdAt: now, updatedAt: now }
  };
}
