...
export function spendingByCategory(categories: Category[], transactions: Transaction[]): SpendingByCategoryRow[] {
  return categories.map((category) => ({
    ...
    total: transactions.filter((transaction) => !transaction.isDeleted).filter((transaction) => transaction.categoryId === category.id).reduce((total, transaction) => total + transaction.amount, 0)
  }));
}
