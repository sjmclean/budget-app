...
export class IndexedTransactionSearchApplicationService {
  ...
  search(
    filters: IndexedTransactionSearchFilters,
  ): IndexedTransactionSearchResult {
    ...
    if (filters.payeeId) {
      where.push("t.payee_id = @payeeId");
      params.payeeId = filters.payeeId;
    }
    if (filters.clearedStatus) {
      where.push("t.cleared_status = @clearedStatus");
      params.clearedStatus = filters.clearedStatus;
    }
    if (filters.dateFrom) {
    ...
    if (filters.amountMin !== undefined) {
      where.push("t.amount >= @amountMin");
      params.amountMin = filters.amountMin;
    }
    if (filters.amountMax !== undefined) {
      where.push("t.amount <= @amountMax");
    ...
    if (filters.tagId) {
      joins.push(
        "INNER JOIN transaction_tag_assignments tta ON tta.transaction_id = t.id",
      );
    ...
    if (filters.text) {
      ...
      where.push(
        "(lower(coalesce(t.memo, '')) LIKE @text OR lower(coalesce(p.name, '')) LIKE @text)",
      );
    ...
    const total = (
      this.sqlite
        .prepare(`SELECT COUNT(DISTINCT t.id) AS count ${from} ${whereSql}`)
