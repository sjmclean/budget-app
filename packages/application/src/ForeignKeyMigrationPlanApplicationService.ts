...
export class ForeignKeyMigrationPlanApplicationService {
  getPlan(): ForeignKeyMigrationPlanStep[] {
    return [
      ...
      {
        table: "transactions",
        references: "budgets/accounts/payees/categories",
        action: "rebuild-table",
        deleteRule: "RESTRICT + soft delete",
      },
      {
        table: "split_transaction_lines",
        references: "transactions(id), categories(id)",
        action: "rebuild-table",
        deleteRule: "CASCADE from transaction only",
      },
      {
        table: "category_months",
        references: "budget_months(id), categories(id)",
        action: "rebuild-table",
        deleteRule: "RESTRICT",
      },
      {
        table: "goals",
        references: "budgets(id), categories(id)",
        action: "rebuild-table",
        deleteRule: "RESTRICT",
      },
      {
        ...
        action: "rebuild metadata tables",
        deleteRule: "CASCADE from transaction",
      },
      {
        table: "bank_import_batch_items",
        references: "bank_import_batches(id), transactions(id)",
        action: "rebuild-table",
        deleteRule: "CASCADE from batch",
      },
    ];
  }
}
