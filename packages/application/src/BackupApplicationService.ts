...
import { canEditBudget, canDeleteBudget } from "../../budget-engine/src/services/permissions.js";
...
export class BackupApplicationService {
  ...
  async recordBackup(userId: string, budgetId: string, filePath: string): Promise<BackupRecord> {
  ...
  async restoreBudget(userId: string, budgetId: string, filePath: string): Promise<BackupRecord> {
    const role = await this.budgetUserRepo.getRole(userId, budgetId);
