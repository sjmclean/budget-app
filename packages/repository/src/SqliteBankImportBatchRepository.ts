...
import type { BankImportBatchItem, BankImportBatchRepository } from "./BankImportBatchRepository.js";
...
export class SqliteBankImportBatchRepository implements BankImportBatchRepository {
  ...
  async addItem(item: BankImportBatchItem): Promise<void> {
    sqlite(this.db).prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(item.id, item.batchId, item.transactionId, item.externalId, item.rawPayee, item.amount, item.date, item.createdAt.getTime());
  }

  async getBatch(batchId: string): Promise<BankImportBatch | null> {
    const row = sqlite(this.db).prepare(`SELECT * FROM bank_import_batches WHERE id = ?`).get(batchId) as any;
    return row ? batchFromRow(row) : null;
  }

  async findItems(batchId: string): Promise<BankImportBatchItem[]> {
    return (sqlite(this.db).prepare(`SELECT * FROM bank_import_batch_items WHERE batch_id = ? ORDER BY created_at ASC`).all(batchId) as any[]).map(itemFromRow);
  }

  async markUndone(batchId: string, undoneAt: Date): Promise<void> {
    sqlite(this.db).prepare(`UPDATE bank_import_batches SET status = 'undone', undone_at = ? WHERE id = ?`).run(undoneAt.getTime(), batchId);
  }
}
...
function sqlite(db: any) {
  ...
  if (!client?.prepare) throw new Error("SqliteBankImportBatchRepository requires a Drizzle better-sqlite3 database with $client");
  return client;
}
