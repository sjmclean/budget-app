...
export class SqlitePayeeRepository implements PayeeRepository {
  ...
  async update(payee: Payee): Promise<void> {
    await this.db.update(payees).set({
      name: payee.name,
      normalizedName: payee.normalizedName ?? payee.name.trim().toLowerCase(),
      isArchived: payee.isArchived ?? false,
      isTransfer: payee.isTransfer ?? false,
  ...
  async archive(payeeId: string): Promise<void> {
    await this.db.update(payees).set({ isArchived: true, updatedAt: new Date() }).where(eq(payees.id, payeeId));
  }
  ...
  async findById(payeeId: string): Promise<Payee | null> {
    const rows = await this.db.select().from(payees).where(eq(payees.id, payeeId));
    return rows[0] ?? null;
  }

  async findByBudget(budgetId: string): Promise<Payee[]> {
    return await this.db.select().from(payees).where(eq(payees.budgetId, budgetId));
  }

  async findActiveByBudget(budgetId: string): Promise<Payee[]> {
    return await this.db.select().from(payees).where(and(eq(payees.budgetId, budgetId), eq(payees.isArchived, false)));
  }

  async findByNormalizedName(budgetId: string, normalizedName: string): Promise<Payee | null> {
    const rows = await this.db.select().from(payees).where(and(eq(payees.budgetId, budgetId), eq(payees.normalizedName, normalizedName)));
    return rows[0] ?? null;
  }

  async search(budgetId: string, query: string): Promise<Payee[]> {
    const normalizedQuery = `%${query.trim().toLowerCase()}%`;
    return await this.db.select().from(payees).where(and(eq(payees.budgetId, budgetId), like(payees.normalizedName, normalizedQuery)));
  }
}
