...
export function ScheduledTransactionsPreview({budgetId,accountId,currencyCode,onViewAll}:{budgetId:string|null;accountId:string;currencyCode:string;onViewAll:()=>void}) {
  ...
  const act=async(item:ScheduledTransactionView,action:"enter"|"skip")=>{if(busyId)return;setBusyId(item.id);setError(null);try{if(action==="skip"){const confirmed=await confirmDialog({message:`Skip "${item.payee}" due ${formatDateForDisplay(item.nextDueDate,dateFormat)}?\n\nThis occurrence will not be added to the register.\nThe schedule will move to its next occurrence.`,confirmLabel:"Skip occurrence"});if(!confirmed)return;}await(action==="enter"?enterSchedule(item):skipSchedule(item));setSchedules(await persistence.listByAccount(accountId));}catch(cause){setError(cause instanceof Error?cause.message:"Scheduled transaction action failed.")}finally{setBusyId(null)}};
  return <section className="register-scheduled-preview" aria-labelledby="scheduled-preview-title">
    ...
    {preview.total ? <div className="register-scheduled-preview-rows">{preview.items.map(item=>{const amount=item.inflow-item.outflow;return <div className="register-scheduled-preview-row" key={item.id}><time dateTime={item.nextDueDate}>{formatDateForDisplay(item.nextDueDate,dateFormat)}</time><strong>{item.payee||"Scheduled transaction"}</strong><span>{item.category}</span><b className={amount>=0?"positive":"negative"}>{money.format(amount)}</b><div><button type="button" disabled={busyId!==null} onClick={()=>void act(item,"enter")}>Enter now</button><button type="button" disabled={busyId!==null} onClick={()=>void act(item,"skip")}>Skip</button></div></div>})}</div> : <p className="register-scheduled-preview-empty">No scheduled transactions in the next {days} days.</p>}
